...
export default function PremiumRouter({
  ...
  // 智能转场类型选择
  const selectTransitionType = useCallback(
    (fromPath: string, toPath: string): TransitionType => {
      // 查找匹配的转场规则
  ...
  const performanceSummary = useMemo(() => {
    ...
    const avgDuration =
      recent.reduce((sum, m) => sum + m.transitionDuration, 0) / recent.length;
  ...
  return (
    ...
    >
      ...
      {/* 性能监控面板 (已禁用 - 2025-11-28) */}
      {false && performanceMonitoring && performanceSummary && (
        <div className="performance-monitor">
          ...
          <div className="monitor-metrics">
            ...
            <div
              className={`status ${
                performanceSummary?.isPerformant ? "good" : "warning"
              }`}
            >
