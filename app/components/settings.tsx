...
export function Settings() {
  ...
  // {{CHENGQI:
  // Action: Added - 自定义模型管理的状态管理
  // Timestamp: 2025-06-18 17:00:00 +08:00
  // Reason: 为自定义模型管理模态框添加状态控制
  // Principle_Applied: SOLID - 单一职责，状态管理分离
  // Optimization: 清晰的状态管理，支持多个提供商
  // Architectural_Note (AR): 扩展设置界面状态管理
  ...
  // Timestamp: 2025-06-18 17:05:00 +08:00
  // Reason: 提供打开和关闭自定义模型管理器的控制函数
  // Principle_Applied: SOLID - 单一职责，功能封装
  // Optimization: 清晰的状态控制逻辑
  // Architectural_Note (AR): 模态框状态管理的最佳实践
  // Documentation_Note (DW): 自定义模型管理器的控制逻辑
  // }}
  const handleOpenCustomModelManager = (provider: ServiceProvider) => {
    setSelectedProvider(provider);
    setShowCustomModelManager(true);
  };

  const handleCloseCustomModelManager = () => {
    setShowCustomModelManager(false);
    ...
    <ErrorBoundary>
      ...
      <div className={styles["settings"]}>
        ...
        <List id={SlotID.CustomModel}>
          ...
          {/* {{CHENGQI:
          // Action: Added - 自定义模型管理界面集成
          // Timestamp: 2025-06-18 17:10:00 +08:00
          // Reason: 为每个服务提供商添加用户友好的自定义模型管理
          // Principle_Applied: SOLID - 单一职责，用户体验优先
          // Optimization: 替代复杂的字符串编辑，提供图形化管理界面
          // Architectural_Note (AR): 在设置界面中集成自定义模型管理
          // Documentation_Note (DW): 为所有服务提供商提供模型管理入口，不依赖 useCustomConfig
          // }} */}
          <ListItem
          ...
          >
            <IconButton
              icon={<ConfigIcon />}
              text={Locale.Settings.CustomModelManager.Manage}
              onClick={() => handleOpenCustomModelManager(accessStore.provider)}
            />
          </ListItem>

          {/* {{CHENGQI:
          // Action: Added - 自定义模型管理器模态框
          // Timestamp: 2025-06-18 18:00:00 +08:00
          // Reason: 将模态框放置在自定义模型管理相关代码附近
          // Principle_Applied: SOLID - 单一职责，逻辑分组
          // Optimization: 更好的代码组织，相关功能放在一起
          // Architectural_Note (AR): 模态框与触发按钮的逻辑分组
          // Documentation_Note (DW): 自定义模型管理器的模态框渲染，位置优化
          // }} */}
          {showCustomModelManager && selectedProvider && (
            <CustomModelManager
              provider={selectedProvider}
              onClose={handleCloseCustomModelManager}
            />
          )}
        </List>
