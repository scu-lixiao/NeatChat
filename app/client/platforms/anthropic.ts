      ...
      return streamWithThink(
        ...
        (text: string, runTools: ChatMessageTool[]) => {
          ...
          try {
            ...
            console.warn("[Anthropic] Failed to parse SSE chunk:", text);
          ...
          if (chunkJson?.type === "content_block_start") {
            if (chunkJson?.content_block?.type === "thinking") {
              ...
              index += 1;
              const id = chunkJson?.content_block?.id || `tool_${index}`;
          ...
          console.warn("[Anthropic] Unhandled SSE event:", chunkJson?.type);
          return { isThinking: false, content: undefined };
        },
